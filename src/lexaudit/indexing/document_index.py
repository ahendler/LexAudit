...
class LegalDocumentIndex:
    ...
    def __init__(self, embeddings: Embeddings):
        ...
        logger.info(
            "LegalDocumentIndex initialized (ChromaDB path: %s)",
    ...
    def index_document(
        self, doc_id: str, full_text: str, metadata: Optional[Dict[str, Any]] = None
    ) -> None:
        """
        ...
        if size <= MAX_DOC_CHARS:
            ...
            logger.info(
                "Indexed large document in ChromaDB: %s (%d chars, %d chunks)",
                doc_id,
                size,
                len(chunks),
            )
    ...
    def search(self, doc_id: str, query: str, k: int = 5) -> List[str]:
        ...
        results = self.large_docs_index.similarity_search(
            query, k=k, filter={"source": doc_id}
        )
